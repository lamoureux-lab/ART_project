...
#Methode d'ecriture
def ecriture_listes_reels(nomFichier,liste,param):
    #On ouvre le fichier
    ...
    for i in range(nbLignes):
        ...
        for k in range(nb_types):
            if liste[3][i] == "%i"%(k+1):
                lignes[i+2] = lignes[i+2] +  types[k]
        for j in range(0,3):
            lignes[i+2] = lignes[i+2] + " %s"%( (float(liste[j][i])*param))	#On y ajoute les points
...
ecriture_listes_reels(nom_out,liste,a)
