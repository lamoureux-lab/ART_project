  ...
  While_diis: do 
    ...
    do_rejected: do 
      ...
      if ( iproc == 0 ) then          ! REPORT DIIS 
        write (*,'(a,3I5,1x,2F11.4,2x,(1p,e14.5,0p),1x,L1)') 'ART DIIS:',  &
      ...
      If_rejected: if ( rejected_step ) then 
         ...
         if ( maxter < 2 ) then   ! DIIS does not have memory to continue
            ...
            if ( .not. ITERATIVE ) then         
               ...
               end_activation = .true. 
               ret = 70000 + pas - 1
               if ( iproc == 0 ) write(*,*) 'ART: DIIS Failed, no memory'
...
subroutine get_solution( maxter, error_vector, solution )
  ...
  call dsysv('U', n, nrhs, matrice, n, interchanges, solution, n, work, lwork, i_err )
                   ! If something fails 
  if ( i_err /= 0 ) then
     if ( iproc == 0 ) write(*,*) 'ART WARNING DIIS: info calculation of solution', i_err
  end if
...
subroutine  apply_lanczos ( liter, saddle_energy, ret )
  ...
  While_lanczos: do
      ...
      if (energy_type == "CPK" .or. energy_type == "GAU") then
      ...
      if_clean_wf: if ( clean_wf ) then
          if ( eigenvalue == eigen_min .and. ftot > EXITTHRESH) then
             ...
             if ( ftot >= EXITTHRESH .and. cw_try_again .and. energy_type .ne. "GAU" .and. energy_type .ne. "CPK" ) then
                cw_try_again = .False.
...
subroutine lanczos_step ( current_energy, a1, liter, get_proj ) 
  ...
  !Local variables
  integer :: i, step_rejected, max_iter
  ...
  max_iter = min((MAXIPERP/5*liter),MAXIPERP)
  call perp_fire(minimized,max_iter)
  ...
  if ( get_proj ) then
     do i = 1, LANCZOS_SCL            ! Lanczos call, we start from the
        ...
        if ( iproc == 0 ) write(*,'(a,3I5,f12.6,f7.2)') &
           & 'ART COLLINEAR:', pas, liter, i, eigenvalue, a1
        if ( a1 > collinear_factor ) exit 
     end do
  end if
                                      ! Debug
  !if ( iproc == 0 ) then                
  ! write(*,*) 'ART: eigenvalue : ', eigenvalue
  ! write(*,"(' ','ART: eigenvals: ',4f12.6)") (eigenvals(i),i=1,4)
  !end if
                                      ! Write 
  call write_step ( 'L', liter, a1, current_energy )

  ! We look for an inflection in the eigenvalue.
  eigen_min = min( eigen_min, eigenvalue )
