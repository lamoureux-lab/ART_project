...
subroutine saddle_converge( ret, saddle_energy )
   ...
   if ( restart .and. ( state_restart == 1 ) ) then 
      ...
      ! Write
      if ( iproc == 0 ) then
         write(*,*) 'ART: Restart  in harmonic well '
         write(*,*) 'ART: kter : ', kter_init
         write(*,*) 'ART: pos: ', pos(1), pos(2), pos(3)
         ...
         if ( kter == KTER_MIN ) then 
            ...
            do i = 1, 1            
               ...
               if ( iproc == 0 ) write(*,'(a,3I5,f12.6,f7.2)') &
                  &   'ART COLLINEAR:', pas, kter, i, eigenvalue, a1
            end do
   ...
   if ( state_restart == 2 ) then 
      ...
      if ( iproc == 0 ) write(*,*) "ART: Restart = 2"
      elseif ( state_restart == 4 ) then 
      diter = iter_restart 
      liter = 1
      switchDIIS= .True.            ! We go to apply_diis.
      if ( iproc == 0 ) write(*,*) "ART: Restart = 4"
   else                             !DEBUG
      if ( iproc == 0 ) write(*,*) "ART: HOUSTON, we've got a problem"
      call end_art () 
   end if 

   call displacement( posref, pos, delr, npart )
   if (iproc==0) write(*,*) "ART: delr npart", delr, npart
   ...
   do i = 1, 5
      ...
      if ( iproc == 0 ) write(*,'(a,2I5,f12.6,f7.2)') &
         &   'ART COLLINEAR:', pas, i, eigenvalue, a1
   ...
   write(*,*) 'ART: Problem with restart and state_restart : '
   write(*,*) 'ART: restart = ', restart, ' state_restart = ', state_restart
   stop
end if If_restart
! _________ 
!                ACTIVATION PART
...
subroutine write_step ( stage, it, a1, energy )
   ...
   if ( iproc == 0 ) then
      ...
      write(*,'(a,i4,2x,a2,i4,1x,(1p,e17.10,0p),1x,2i3,4f12.6,1x,1f10.4,i4,i6)') &
         &   " ART:", pas, etape, it, energy,  m_perp, try, ftot, fpar, fperp,   &
...
subroutine end_report ( success, ret, saddle_energy )
   ...
   if ( iproc == 0 ) then
      ...
      write(*,"(/' ','ART: SADDLE',i5, a10,' |ret ',i6,' |delta energy= '," //  &
         &    "f9.4, ' |force_(tot,par,perp)= ', 3f10.4," //       &
         &    "' |eigenval=',f9.4,' |npart= ', i4,' |delr= ', f8.3,' |evalf='," // &
         &    "i6, ' |')")                                          & 
         & mincounter, adjustr(converg), ret, delta_e, ftot,  &  
