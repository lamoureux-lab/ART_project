...
subroutine restart_states( istatus, ieventcurrent, iterations, iatp )
  ...
  do i = 1, NATOMS
     Atom(i) = typat(i)
                                      ...
                                      ! DEBUG 
  if ( iproc == 0 ) then
     write(*,*) 'ART: restart file'
     write(*,*) 'ART: central_atom ', central_atom  
     write(*,'(a,(1p,e17.10,0p))') ' ART: total energy: ', total_energy
     write(*,'(a,(1p,e17.10,0p))') ' ART: ref   energy: ', ref_energy 
     write(*,'(a,3f20.8)') ' ART: xref(1): ', xref(1), yref(1), zref(1)
     write(*,'(a,3f20.8)') ' ART: x(1)   : ', x(1), y(1), z(1)
     write(*,'(a,3f20.8)') ' ART: x(NATOMS): ', x(NATOMS), y(NATOMS), z(NATOMS)
 
     if ( istatus == 2 .or. istatus == 4 ) then
     write(*,*) 'ART: previous_norm : ', diis_norm_restart
     write(*,*) 'ART: last line ',maxter_r,eigen_min_r,eigenvalue_r,nsteps_after_eigen_min_r 
     end if 
     write(*,*) 'ART: Reading atomic input positions from RESTART file,'
     write(*,*) 'ART: please, ignore the next message. '
  end if
