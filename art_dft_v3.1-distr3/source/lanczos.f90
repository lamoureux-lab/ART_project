...
!!    Copyright (C) 2010 Eduardo Machado
!!    Copyright (C) 2014 Laurent Karim Béland and Normand Mousseau
...
subroutine lanczos( maxvec, new_projection, produit )
     ...
     do i = 1, natoms, 1
        if ( constr(i) == 0 ) then
           do
              ...
              dy(i) = 0.5d0 - ran3()
              dz(i) = 0.5d0 - ran3()
              ! ensure that the displacement is isotropic
  ...
  do ivec = 2, maxvec - 1
     ...
     ! Gramm-Schmidt orthogonalization to avoid "ghosts" in the Lanczos matrix
     do j = 1, 4        
          do i = 1, ivec
...
! Check: impose that the center of mass at zero. 
