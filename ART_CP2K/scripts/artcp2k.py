...
parser.add_argument('-f', '--cp2k_input', nargs = '+', help = 'cp2k input file')
...
submission_script_template =  join(dirname(relpath(__file__)), 'cp2k_psi.sub')
exec_script_template       =  join(dirname(relpath(__file__)), 'execute_cp2k.py')
create_header_template     =  join(dirname(relpath(__file__)), 'update_cp2k_input.py')
cp2k_art_template          =  join(dirname(relpath(__file__)), 'cp2k_art.sh')
submission_script          =  'cp2k_psi.sub'
exec_script                =  'execute_cp2k.py'
update_header              =  'update_cp2k_input.py'
cp2k_art                   =  'cp2k_art.sh'
refconfig                  =  'refconfig.dat'
filecounter                =  'filecounter'
shared_log                 =  '../shared_log/'
files_to_run               =  args.cp2k_input
my_dict                    =  {}
    ...
    for directory in my_dict.values():        
        with open (submission_script_template) as f:
            with open (join(directory, submission_script), 'w+') as m:
                ...
                calling_art = 'csh ' + cp2k_art + ' > ' + 'output.log'
...
def update_cp2k_input():
    for directory in my_dict.values():
        with open (create_header_template) as f:
            with open (join(directory, update_header), 'w+') as m:
                for line in f:
                    m.write(line)
...
if __name__ == '__main__':
    ...
    create_cp2k_art(zipped_atomic_list, min_sad_natoms_read)
    create_refconfig()
    create_filecounter()
    submitting_scripts()
